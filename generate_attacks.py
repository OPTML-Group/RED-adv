    ...
    for act in ['elu','relu','tanh']:
        ...
        # for pr in ['0.0']:
            ...
            for i in [0]:
                ...
                atk = torchattacks.PGD(model, eps = 8/255, alpha=2/255, steps=10) # PGD_linf
                atk = torchattacks.CW(model) # CW
                atk = torchattacks.PGDL2(model, eps = 0.5, alpha=0.1, steps=10) # PGD_l2, view -> reshape
                atk = torchattacks.AutoAttack(model, norm='Linf', eps = 8/255) # AutoAttack
                atk = torchattacks.FGSM(model, eps = 8/255) # FGSM (l_inf)
                atk.set_normalization_used(mean = CIFAR_MEAN_1, std = CIFAR_STD_1) # normalization if data is already normalized
                for ims, labs in loaders['test']:
                    with autocast():
                        ...
                        print("clf correct & atk rate",sum(idx)/len(idx)*100, '%')
