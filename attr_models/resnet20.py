...
class BasicBlock(nn.Module):
    ...
    def __init__(self, args, in_planes, planes, stride=1, option='A'):
        ...
        self.conv1 = conv(args, in_planes, planes, kernel_size=3, stride=stride, padding=int((3-1)/2)) # if ks = 3 then padding = 1; if ks = 1 then padding = 0; if ks = 5 then padding = 2
        
        self.bn1 = nn.BatchNorm2d(planes)
        # self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv2 = conv(args, planes, planes, kernel_size=3, stride=1, padding=int((3-1)/2)) # if ks = 3 then padding = 1; if ks = 1 then padding = 0; if ks = 5 then padding = 2
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.conv1 = conv(args, 3, 16, kernel_size=3, stride=1, padding=int((3-1)/2)) # if ks = 3 then padding = 1; if ks = 1 then padding = 0; if ks = 5 then padding = 2
