...
def _zosignsgd(X, y, model, steps=500, eta=0.0005, eps=8/255,
    ...
    with torch.no_grad():
        for i in range(steps):
            ...
            gs_t = gs_t / q  # due to the sign mark next line. /q does not work here
...
class ZoSignSgd:
    ...
    def __call__(self, image, target):
        return _zosignsgd(image, target, self.model, mean=self.mean, std=self.std, eps=self.eps)
