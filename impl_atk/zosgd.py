        ...
        for i in range(steps):
            ...
            for _ in range(q // est_time):
                ...
                if est_time == 1:
                    ...
                    est_deriv = (
                        loss_fn(model((x_1 - mean) / std), y) - X_adv_loss
                    ) / mu
                elif est_time == 2:
                    x_1 = X_adv + mu * exp_noise
                    ...
                    est_deriv = (
                        (
                            loss_fn(model((x_1 - mean) / std), y)
                            - loss_fn(model((x_2 - mean) / std), y)
                        )
                        / 2
                        / mu
                    )
                else:
                    raise NotImplementedError(f"est_time {est_time} not implemented")
