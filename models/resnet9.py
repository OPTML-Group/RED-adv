...
class ResNet9(nn.Module):
    def __init__(self, num_classes=10, kernel_size=3, act_func="relu"):
        ...
        self.model = torch.nn.Sequential(
            # if ks = 3 then padding = 1; if ks = 1 then padding = 0; if ks = 5 then padding = 2
            conv_bn(3, 64, kernel_size=kernel_size, stride=1,
                    padding=int((kernel_size-1)/2), act_func=act_func),
            conv_bn(64, 128, kernel_size=kernel_size, stride=2, padding=int(
                (kernel_size+1)/2), act_func=act_func),  # if ks = 3 then padding = 2; if
            Residual(torch.nn.Sequential(
                conv_bn(128, 128, kernel_size=kernel_size, padding=int((kernel_size-1)/2), act_func=act_func),
                conv_bn(128, 128, kernel_size=kernel_size, padding=int((kernel_size-1)/2), act_func=act_func))),
                conv_bn(128, 256, kernel_size=kernel_size, stride=1, padding=int((kernel_size-1)/2), act_func=act_func),
            torch.nn.MaxPool2d(2),
            Residual(torch.nn.Sequential(
                conv_bn(256, 256, kernel_size=kernel_size, padding=int((kernel_size-1)/2), act_func=act_func),
                conv_bn(256, 256, kernel_size=kernel_size, padding=int((kernel_size-1)/2), act_func=act_func))),
